...
describe('backup download tokens', () => {
  const filename = 'em-backup-2026-06-02-1430.zip';
  ...
  it('rejects tokens for a different user or filename', async () => {
    ...
    assert.equal(
      await verifyBackupDownloadToken(token!, userId, 'em-backup-2026-06-02-1500.zip'),
      false
    );
  });
  ...
  it('uses a separate short-lived HttpOnly cookie for each backup', () => {
    assert.notEqual(
      ...
      backupDownloadCookieName('em-backup-2026-06-02-1500.zip')
    );
